...
The mark at the top-left of the header is the app's icon, and clicking it opens
**App icon**, where anyone in the house can put a family photo there instead.
The photo is centre-cropped to a square and shrunk to 512px in the browser, then
stored the same way avatars are — a private object plus a short-lived signed
URL — with an inline copy kept in `localStorage`.

That one upload changes three things:

| Where | How |
| --- | --- |
| The header mark | Directly, on every device, as soon as it saves |
| The browser tab | `<link rel="icon">`, swapped at runtime, and re-applied before first paint on later visits |
| The iPhone home screen | `<link rel="apple-touch-icon">`, read by Safari the next time the app is added |

An **Android** launcher icon comes from `manifest.webmanifest`, which is a
static file in the export, so an installed Android app keeps the default house
mark. Changing that one means editing
...
has been run the app falls back to storing it in the browser and says so in the
dialog — the icon still works, it just does not travel.

### Installing it as an app

**Download app** in the header installs the dashboard properly: its own icon,
its own window, no browser chrome. It uses the platform's own install prompt
(`beforeinstallprompt`), captured in the layout's boot script because Chrome
fires it before React exists.

The button removes itself once there is nothing left to offer — after the app
is installed, when it is already running standalone, and on browsers that
cannot install at all. On iOS, where there is no install API, it opens the
three-step Add to Home Screen explanation instead.

`public/sw.js` is what makes the app installable, and it is deliberately
network-first: this is a live view of a shared database, and a cache that won
over the network would show yesterday's calendar. It keeps a copy of the shell
only so a reload with no signal paints something.
...
role read/write on the ten `family_*` tables.

**In practice: anyone with the site URL and that key can read and write this
family's data** — and that now includes family photos, uploaded files, voice
recordings and the app icon. The storage bucket is private and served through
